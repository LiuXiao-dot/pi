...
- Room status board (the no-active-reply view) now interleaves the user prompt bubble with its session status row in chronological order. Previously `renderRoomStatusBoard` rendered every user bubble first and every status row after, so a turn's question appeared at the top of the panel and its result at the bottom; with many turns the two halves drifted far apart and visually decoupled. `ReplyEntry` now carries optional `userText` / `userMeta` fields populated both on live `prompt` send and during `ingestJoinHistory`, the legacy `roomUserMessages` registry was removed, and entries are sorted by `startedAt` so role/compaction rows still slot in by time between sessions.
